//单独封装打开页面
window.openview = function (obj){
	var t = obj.top || '0px';//距离顶部距离
	var b = obj.bottom || '0px';//距离底部距离
	var e = obj.extrasobj || {};//页面间传值
	var c = obj.create || false;//是否重新打开已有的webview
	var aniShow = obj.aniShow || 'fade-in';//是否重新打开已有的webview
	if(obj.reOpen){//如果传 reOpen 参数，就先关闭已经打开的webview
		if(plus.webview.getWebviewById(obj.id)){
...
//下拉刷新
window.downRe = function(backfun,height){
	document.getElementsByTagName('body')[0].style.minHeight = '500px';
...
function closeLoad(){
	var time3 = setTimeout(function(){
		mui('#loadingdiv')[0].style.height = '0px';
		var time4 = setTimeout(function(){
			mui('#loadingdiv')[0].style.display = 'none';
		},300)
	},300)
}
var loadstr = '<div id="loadingdiv" style="text-align: center;font-size: 14px;height:30px;background-color: #f2f1ee;'+
		...
		a.openWindow = function(b, c, f) {
			...
			if(window.plus) {
				...
				if(l.autoShow && (h = plus.nativeUI.showWaiting(l.title, l.options)), f = a.extend(f, {
					...
					b ? j.addEventListener("titleUpdate", m, !1) : m()
				}
				return j
			}
